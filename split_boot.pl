...
** p = (dtree_size + page_size - 1) / page_size
=cut
...
my $P = int(($DTREE_SIZE + $PAGE_SIZE - 1) / $PAGE_SIZE);
...
printf "kernel offse: 0x%08x, size:0x%08x\n", $k_offset, $KERNEL_SIZE;

# The ramdisk is always there
print "Writing $r_file ...";
&dump_file($IMAGE_FN, $r_file, $r_offset, $RAMDISK_SIZE);
printf "ramdisk offse: 0x%08x, size:0x%08x\n", $r_offset, $RAMDISK_SIZE;
...
unless ($SECOND_SIZE == 0) {
    ...
    printf "second stage  offse: 0x%08x, size:0x%08x\n", $s_offset, $SECOND_SIZE;
}

# The device tree is always there
print "Writing $dt_file ...";
&dump_file($IMAGE_FN, $dt_file, $d_offset, $DTREE_SIZE);
printf "device tree offset: 0x%08x, size:0x%08x\n", $d_offset, $DTREE_SIZE;
print " complete.\n";
...
sub parse_header {
    ...
    printf "device tree size: %d (0x%08x)\n", $dt_size, $dt_size;
