...
stream = os.popen("readelf -S %s | grep '\s\.text'" % vmlinux)
for line in stream:
...
stream = os.popen("readelf -s %s | grep '__v7_setup_stack'" % vmlinux)
...
stream = os.popen("readelf -S %s | grep '\s\.rodata'" % vmlinux)
...
stream = os.popen("readelf -l %s | grep 'LOAD'" % dumpfile)
for line in stream:
...
# check verion first
# get linux_banner
stream = os.popen("readelf -s %s | grep 'linux_banner'" % vmlinux)
...
banner_offset = int(banner_addr[1:], 16) - loadaddr
